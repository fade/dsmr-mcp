...
See also: [[file:index.org]], [[file:architecture.org]], [[file:configuration.org]],
[[file:operations.org]].
...
"Remote dispatch (non-localhost) with TLS + auth"), not a v1 control. See
[[file:configuration.org]] for =DSMR_ALLOW_REMOTE= and the sandbox configuration.
...
| Client → dispatcher (TCP)             | Bind =127.0.0.1= (loopback)           | Loopback-only bind by default (=src/transport/tcp.lisp=, TRANS-04)          | A non-loopback =DSMR_BIND= is refused at startup (=%check-remote-bind=, =src/run.lisp=) unless =DSMR_ALLOW_REMOTE= is set; the operator chooses the address with =DSMR_BIND=. No auth on the connection — see the STRIDE register.                                                                                                                                                                                                                                                                                                                                                                                        |
| Client → dispatcher (Streamable HTTP) | Bind =127.0.0.1=; loopback-only CORS  | Loopback-only bind + loopback-only =Origin= policy (=src/transport/http.lisp=, TRANS-04) | The CORS allow-list is loopback-only unconditionally: a non-loopback Origin never receives =Access-Control-Allow-Origin=, regardless of =DSMR_ALLOW_REMOTE= (which gates only the bind address).                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
=DSMR_ALLOW_REMOTE= is the single, documented opt-out that permits a
network bind off loopback. When it is unset (the default), a non-loopback
=DSMR_BIND= is refused at startup, so the TCP and HTTP transports can bind
only =127.0.0.1=. When it is set, the operator may point =DSMR_BIND= at a
routable interface (e.g. =0.0.0.0=) — the flag itself changes no address,
and the HTTP CORS policy stays loopback-only either way. The flag adds *no
authentication and no transport encryption* — it only removes the
loopback restriction on the bind address. Enabling it is an
...
See [[file:configuration.org]] for the bind and remote-gate settings.
...
| T-06      | Denial of service                   | A single hermetic call running unbounded (hung eval, infinite loop)                                                                                   | mitigate                     | Every worker call carries a soft eval budget (default 120 s, =DSMR_WORKER_EVAL_TIMEOUT=) enforced inside the worker; on overrun past that budget plus a kill grace (=DSMR_WORKER_KILL_GRACE_SECONDS=, default 5 s) the parent (=src/hermetic/pool.lisp=) *hard-kills* the worker with SIGKILL, recording the kill as a crash against the breaker (SAFETY-05). This control is hermetic-only; the attached image is the operator's own and is not force-killed.                                                                       |
