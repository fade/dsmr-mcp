...
Force-drop and reopen this session's connection to the 
...
Report the health of whichever backend this server is using: the 
...
Inspect one coordination bus in depth: which broker is serving 
...
Leave a coordination bus cleanly. This agent reads out whatever 
...
Broadcast a message onto the coordination bus. All subscribed 
...
Receive messages from the coordination bus that this agent has 
...
#+begin_src json
{
  "properties": {
    ...
    "limit": {
      "description": "Maximum number of messages to deliver in this \ncall (default 20). Delivery is oldest-first and paginated: the cursor advances \nonly over what was delivered, so the remainder is not lost and is picked up by \nthe next call.",
      "type": "integer"
    },
    "skip_to_head": {
...
Read and change the roster of a coordination bus: list who is on 
...
Report the coordination bus state for this agent: whether a broker 
...
Structure-aware edit of a comment in a Lisp source file. 
...
Reset the local backends in place (rung 1 of the recovery ladder). 
...
Direct a local reset at a NAMED peer agent in this project's bus 
...
Restart the whole dsmr-mcp server process (rung 3, the TERMINAL 
