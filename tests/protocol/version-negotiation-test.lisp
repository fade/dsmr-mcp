...
;;;;
;;;; Version negotiation matrix test.
;;;; Supported versions are "2025-06-18" and "2025-03-26" only.
;;;; Unsupported versions return server-highest in result (NOT an error).
...
(define-test supported-version-2025-06-18-echoed
  "Requesting \"2025-06-18\" returns \"2025-06-18\" in result."
  (is equal "2025-06-18" (%negotiated-version "2025-06-18")))

(define-test supported-version-2025-03-26-echoed
  "Requesting \"2025-03-26\" returns \"2025-03-26\" in result."
  (is equal "2025-03-26" (%negotiated-version "2025-03-26")))

(define-test dropped-version-2024-11-05-returns-highest
  "\"2024-11-05\" is not in the supported set (dropped for maintenance reasons).
Response must be a RESULT (not an error) containing the server's highest version
\"2025-06-18\". Diverges from cl-mcp which returns -32602."
...
(define-test future-version-returns-highest
  "A future version string unknown to this server returns a RESULT with the
server's highest version. Clients may proceed if they choose to."
  (let ((obj (%negotiate "2099-01-01")))
    (false (%is-error-response obj))
    (is equal "2025-06-18" (gethash* obj "result" "protocolVersion"))))

(define-test empty-protocol-version-returns-highest
  "When protocolVersion is an empty string the server picks its highest."
  (let ((obj (%negotiate "")))
    (false (%is-error-response obj))
    (is equal "2025-06-18" (gethash* obj "result" "protocolVersion"))))
