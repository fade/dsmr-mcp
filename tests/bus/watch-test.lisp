...
(defpackage #:dsmr-mcp/tests/bus/watch-test
  ...
  (:import-from #:dsmr-bus-watch/src/bus/watch
                ...
                #:%resolve-baseline
                #:%cursor-path
                ;; internal: the heartbeat helpers. The pure liveness decision
                ;; and its file-backed classifier both carry behavior a caller
                ;; depends on but cannot observe through main alone.
                #:%beat-path
                #:%write-beat
                #:%beat-status
                #:%beat-liveness
